...
class ClipperPipeline:
    ...
    def run(
        ...
        try:
            ...
            w_count = getattr(transcript, "word_count", sum(len(s.words) for s in transcript.segments))
            if w_count < 30:
                msg = f"Insufficient spoken dialogue ({w_count} words in {transcript.duration:.0f}s); skipping non-verbal video"
            ...
            for idx, candidate in enumerate(candidates):
                ...
                try:
                    ...
                    caption_result = generate_clip_captions(transcript, candidate, caption_base, provider=self.provider)
